...
class DacliWeb_Customizer_FrameWork {
	 ...
	 *
	 * DacliWeb_Customizer_FrameWork constructor.
	 */
	...
	public function register_panels_sections_controls( $wp_customize ) {
		...
		$wp_customize->register_section_type( 'DacliWeb_WP_Customize_Separator' );
		$wp_customize->register_panel_type( 'DacliWeb_WP_Customize_Panel' );
		...
		if ( current_theme_supports( 'custom-background' ) ) {
			...
			$wp_customize->get_setting( 'background_color' )->sanitize_callback = array(
				'DacliWeb_Customizer_FrameWork_Sanitizes',
				'sanitize_alpha_color',
			);

			add_filter(
				'customize_sanitize_background_color',
				array( 'DacliWeb_Customizer_FrameWork_Sanitizes', 'sanitize_alpha_color' ),
				10,
				2
			);
		}
		...
		DacliWeb_Customize_Base_Control::add_control(
			'spacious-navigate',
			array(
				'callback'          => 'DacliWeb_Navigate_Control',
				'sanitize_callback' => array(
					'DacliWeb_Customizer_FrameWork_Sanitizes',
					'sanitize_false_values',
				),
		...
		DacliWeb_Customize_Base_Control::add_control(
			'spacious-title',
			array(
				'callback' => 'DacliWeb_Title_Control',
			)
		);
		...
		DacliWeb_Customize_Base_Control::add_control(
			'spacious-dimensions',
			array(
				'callback' => 'DacliWeb_Dimensions_Control',
			)
		);
		...
		DacliWeb_Customize_Base_Control::add_control(
			'spacious-upgrade',
			array(
				'callback' => 'DacliWeb_Upgrade_Control',
			)
		);
		...
		DacliWeb_Customize_Base_Control::add_control(
			...
			array(
				'callback' => 'DacliWeb_Fontawesome_Control',
				'sanitize_callback' => array(
					'DacliWeb_Customizer_FrameWork_Sanitizes',
					'sanitize_radio_select',
				),
	...
	public function register_panel( $config, $wp_customize ) {
		...
		$wp_customize->add_panel(
			new DacliWeb_WP_Customize_Panel(
				$wp_customize,
				$config['name'],
				$config
			)
	...
	public function register_section( $config, $wp_customize ) {
		...
		$section_callback = isset( $config['section_callback'] ) ? $config['section_callback'] : 'DacliWeb_WP_Customize_Section';
	...
	public function register_sub_control_setting( $config, $wp_customize ) {
		...
		// For adding settings.
		$sanitize_callback = isset( $config['sanitize_callback'] ) ? $config['sanitize_callback'] : DacliWeb_Customize_Base_Control::get_sanitize_callback( $config['control'] );
		...
		// For adding controls.
		$control_type = DacliWeb_Customize_Base_Control::get_control_instance( $customize_config['control'] );
	...
	public function register_setting_control( $config, $wp_customize ) {
		...
		// For adding controls.
		$control_type   = DacliWeb_Customize_Base_Control::get_control_instance( $config['control'] );
...
return new DacliWeb_Customizer_FrameWork();
