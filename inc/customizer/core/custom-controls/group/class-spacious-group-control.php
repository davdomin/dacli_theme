 ...
 * Class DacliWeb_Group_Control
 */
class DacliWeb_Group_Control extends DacliWeb_Customize_Base_Additional_Control {
	...
	public function enqueue() {
		...
		wp_localize_script(
			...
			'DacliWebCustomizerControlGroup',
			array(
	...
	public function to_json() {
		...
		if ( isset( DacliWeb_Customizer_FrameWork::$group_configs[ $this->name ]['tabs'] ) ) {

			$tab = array_keys( DacliWeb_Customizer_FrameWork::$group_configs[ $this->name ]['tabs'] );

			foreach ( $tab as $key => $value ) {
				$config['tabs'][ $value ] = wp_list_sort( DacliWeb_Customizer_FrameWork::$group_configs[ $this->name ]['tabs'][ $value ], 'priority' );
			}
		} else {

			if ( isset( DacliWeb_Customizer_FrameWork::$group_configs[ $this->name ] ) ) {
				$config = wp_list_sort( DacliWeb_Customizer_FrameWork::$group_configs[ $this->name ], 'priority' );
			}
		}
