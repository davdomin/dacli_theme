	...
	function ( $ ) {
		...
		wp.customize.controlConstructor['spacious-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initDacliWebBackgroundControl();

			},

			initDacliWebBackgroundControl : function () {
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( DacliWebCustomizerControlBackground.placeholder );
					}
		...
		wp.customize.controlConstructor['spacious-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					...
					control.initDacliWebFontawesomeControl();
				},

				initDacliWebFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['DacliWebCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
	...
	function ( $ ) {
		...
		wp.customize.controlConstructor['spacious-group'] = wp.customize.Control.extend( {
						...
						if ( $this.hasClass( 'open' ) ) {
							...
							if ( is_loaded ) {
								...
								var fields     = control.params.spacious_fields,
								    modal_wrap = $( DacliWebCustomizerControlGroup.group_modal_tmpl ),
				...
				_.each(
					...
					function ( attr, index ) {
						...
						if ( (
							...
							attr.languages = DacliWebCustomizerControlTypographySubsets;
						}
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( DacliWebCustomizerControlTypography ) ) {
					return DacliWebCustomizerControlTypography;
				}
