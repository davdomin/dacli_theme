<?php
/**
 * DacliWeb enqueue CSS and JS files.
 *
 * @package    ale8bits
 * @subpackage DacliWeb
...
function spacious_scripts_styles_method() {
	...
	// Dacli custom styles (loads after main stylesheet to override defaults).
	wp_enqueue_style(
		'dacli-custom',
		get_template_directory_uri() . '/assets/css/custom.css',
		array( 'spacious_style' ),
		'1.0.0'
	);
	 ...
	 */
	add_filter( 'spacious_dynamic_theme_css', array( 'DacliWeb_Dynamic_CSS', 'render_output' ) );

	// Enqueue required Google font for the theme.
	DacliWeb_Generate_Fonts::render_fonts();
...
function spacious_get_fonts() {
	...
	DacliWeb_Generate_Fonts::add_font( $spacious_content_font_typography['font-family'] );
	DacliWeb_Generate_Fonts::add_font( $spacious_titles_font_typography['font-family'] );
}
